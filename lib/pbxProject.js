...
pbxProject.prototype.setBuildSettings = function(settingName, settingValue) {
    var configurations = nonComments(this.pbxXCBuildConfigurationSection()),
...
pbxProject.prototype.setIOSTargetDevice = function(deviceType){
    var device = deviceType.toLowerCase(),
        DEVICE_SETTING_NAME = 'TARGETED_DEVICE_FAMILY',
        IPHONE_FAMILY = '"1"',
        IPAD_FAMILY = '"2"',
        UNIVERSAL = '"1,2"';

    if(device === "iphone"){
        this.setBuildSettings(DEVICE_SETTING_NAME, IPHONE_FAMILY);
    } else if(device === "ipad"){
        this.setBuildSettings(DEVICE_SETTING_NAME, IPAD_FAMILY);
    } else {
        this.setBuildSettings(DEVICE_SETTING_NAME, UNIVERSAL);
    }
}
